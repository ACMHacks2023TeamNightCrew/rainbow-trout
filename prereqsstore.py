...
depts = [dept.text.split(" ")[0].split("-")[0] for dept in deptElements] + ["College"]
...
with open("prereqs.csv", "w") as f:
    for i in range(len(courseNames)):
        f.write(courseNames[i]["course"])
        if len(courseNames[i]["prereqs"]) != 0:
            f.write(", ")
            f.write(", ".join(courseNames[i]["prereqs"]))
        f.write("\n")